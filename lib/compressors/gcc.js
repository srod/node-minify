...
function compressGCCJS(settings, content, callback) {
  ...
  /**
   * Write GCC sourceMaps
   * If the createSourceMap options is passed we'll write the sourceMap file
  ...
  */
  ...
  if (settings.options.createSourceMap) {
    var sourceMapOutput = (typeof settings.options.createSourceMap == 'boolean') 
      ? (settings.output + '.map') 
