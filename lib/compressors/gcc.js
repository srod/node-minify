...
function compressGCCJS(settings, content) {
  return new Promise(function(resolve) {
    ...
    var contentMinified = gcc.compile(flags); // TODO add settings
...
function applyOptions(flags, options){
  ...
  Object.keys(options)
    // filter invalid option keys
    .filter(option => allowedFlags.includes(option))
    // apply valid options to flags
