var minify = (function (undefined) {
    ...
    var minify = function (options) {
        ...
        if (options.fileIn.indexOf('*') > -1) {
            options.fileIn = glob.sync(options.fileIn, null);
        }
        ...
        compress: function() {
            ...
            switch (this.type) {
                ...
                case 'gcc':
                    ...
                    command = 'java -client -d32 -jar -Xss2048k "' + __dirname + '/google_closure_compiler-v20130411.jar" ' + fileInCommand.join(' ') + ' --language_in=' + (this.language || 'ECMASCRIPT3') + ' --js_output_file="' + this.fileOut + '" ' + this.options.join(' ');
                    break;
                case 'uglifyjs':
