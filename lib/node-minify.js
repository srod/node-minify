    ...
    compress: function() {
      ...
      switch (this.type) {
        ...
        case 'gcc':
          ...
          command = 'java -server -XX:+TieredCompilation -jar -Xss2048k "' + __dirname + '/google_closure_compiler-v20130411.jar" ' + fileInCommand.join(' ') + ' --warning_level=QUIET --language_in=' + (this.language || 'ECMASCRIPT3') + ' --js_output_file="' + this.fileOut + '" ' + this.options.join(' ');
          break;
        case 'uglifyjs':
          command = '"' + getPath('uglifyjs') + '" --output "' + this.fileOut + '" ' + ' "' + this.fileIn + '" ' + this.options.join(' ') + (this.copyright ? '--comments' : '');
          break;
        case 'sqwish':
          command = '"' + getPath('sqwish') + '" "' + this.fileIn + '" -o "' + this.fileOut + '" ' + this.options.join(' ');
          break;
        case 'clean-css':
          command = '"' + getPath('cleancss') + '" "' + this.fileIn + '" -o "' + this.fileOut + '" ' + this.options.join(' ');
          break;
        case 'csso':
